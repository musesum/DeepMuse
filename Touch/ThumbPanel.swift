...
class ThumbPanel: ThumbView {
    ...
    var autoHide: Timer?    // hide panel, if still showing after delay
    var autoHiding = false  // workaround for autoHide not cancelling when it should
    ...
    convenience init(_ touchView_:UIView, tr3: Tr3) {

        self.init(touchView_)
    ...
    func makeControlPanel(for tr3: Tr3) {
        ...
        for child in tr3.children {
            ...
            case "shader"   : MetalTr3.shared.makeShader(for: child)
            default: break
            }
        }
    }
    ...
    @objc func showPanel(_ from:String) {
        ...
        case .hidden:
            ...
            removeFromSuperview()
            touchView.insertSubview(self, at:0)
    ...
    func resetAutoHide(delay: CFTimeInterval) {

        func maybeHidePanel(_ timer:Timer) {
            if self.autoHiding {
                self.autoHiding = false
                switch state {
                case .showing: hidePanel("autoHide")
                default: break
                }
            }
        }
        autoHide = Timer.scheduledTimer(withTimeInterval: delay, repeats: false, block: maybeHidePanel)
        autoHiding = true
    }
    func cancelAutohide() {     
        autoHiding = false
    ...
    func thumbTouched(_ phase: UITouch.Phase) {
        ...
        case .ended, .cancelled:
            ...
            resetAutoHide(delay: AutoHideDuration)
            fadeBezel(0.6)
    ...
    func dotTap1() {
        ThumbPrint("Panel_dotTap1")
        showPanel(after:0.5)
    }
    func dotTap2() {
        ThumbPrint("Panel_dotTap2")
    }
