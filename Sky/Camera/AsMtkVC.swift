...
/**
 * A `UIViewController` that allows quick and easy rendering of Metal textures. Currently only supports textures from single-plane pixel buffers, e.g. it can only render a single RGB texture and won't be able to render multiple YCbCr textures. Although this functionality can be added by overriding `AsMtkVC`'s `willRenderTexture` method.
 */
open class AsMtkVC: UIViewController {
...
extension AsMtkVC: MTKViewDelegate {
